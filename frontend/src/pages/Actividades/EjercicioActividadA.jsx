...
const EjercicioActividadA = () => {
    const { idEjercicioCurso } = useParams();
                ...
                try {
                    ...
                    if (moveResult) {
                        ...
                        if (moveIndex >= expectedMoves.length - 1) {
                            ...
                            setMessage(`Tu turno: ${moveIndex + 2}/${expectedMoves.length}`);
                        }
        ...
        const fetchExercise = async () => {
            try {
                ...
                const response = await axios.get(
                    `${import.meta.env.VITE_API_URL}/api/admin/ver-ejercicio-to-Alumno/${idEjercicioCurso}`,
    ...
    const handleSubmitExercise = async () => {
        ...
        try {
            ...
            const now = new Date();
            const diaEntrega = now.getDate();
            const mesEntrega = now.getMonth() + 1; // Los meses son 0-indexados
            const añoEntrega = now.getFullYear();
            const horaEntrega = `${now.getHours()}:${now.getMinutes()}:${now.getSeconds()}`;

            const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/crear-ejercicio-alumno`, {
                idUsuario: decoded.userId,
                idEjercicioCurso,
                estatusEjercicio: 'entregado',
                diaEntrega,
                mesEntrega,
                añoEntrega,
                horaEntrega
            ...
            if (response.data.message === 'Ejercicio del alumno creado exitosamente.') {
                setMessage('¡Tarea entregada con éxito! Se ha notificado al maestro.');
            } else {
                setMessage('La tarea se entregó, pero hubo un problema al notificar al maestro.');
            }
        } catch (error) {
            console.error('Error al entregar la tarea:', error);
            setMessage(`Error al entregar la tarea: ${error.response?.data?.message || error.message}`);
        }
    };
    ...
    return (
        ...
        <div className="ejercicio-actividad-a-container">
            <div className="ejercicio-actividad-a-main-content">
                <div className="ejercicio-actividad-a-board-and-info-container">
                    {/* Contenedor del tablero */}
                    <div className="ejercicio-actividad-a-board-container">
                        <div className="ejercicio-actividad-a-controls">
                        <div className="ejercicio-actividad-a-theme-selector-container">
                            <button 
                            className="ejercicio-actividad-a-theme-dropdown-btn" 
                            ...
                            {showThemeDropdown && (
                            <div className="ejercicio-actividad-a-theme-dropdown">
                                <button 
                                onClick={() => {
                                    setBoardTheme('green');
                                    setShowThemeDropdown(false);
                                }}
                                className={boardTheme === 'green' ? 'ejercicio-actividad-a-active' : ''}
                                >
                                <span className="ejercicio-actividad-a-theme-preview ejercicio-actividad-a-green"></span>
                                Verde Clásico
                                </button>
                                <button 
                                onClick={() => {
                                ...
                                className={boardTheme === 'brown' ? 'ejercicio-actividad-a-active' : ''}
                                >
                                <span className="ejercicio-actividad-a-theme-preview ejercicio-actividad-a-brown"></span>
                                Madera Clásica
                                </button>
                                <button 
                                onClick={() => {
                                ...
                                className={boardTheme === 'blue' ? 'ejercicio-actividad-a-active' : ''}
                                >
                                <span className="ejercicio-actividad-a-theme-preview ejercicio-actividad-a-blue"></span>
                                Azul Marino
                                </button>
                            </div>
                            )}
                        ...
                        <div className="ejercicio-actividad-a-board-wrapper" style={{ width: boardSize, height: boardSize }}>
                        <Chessboard 
                            position={game.fen()} 
                            onPieceDrop={onDrop}
                    ...
                    <div className="ejercicio-actividad-a-right-panel">
                        {/* Mensajes movidos aquí arriba */}
                        <div className="ejercicio-actividad-a-message">
                        {message}
                        {completed && <span className="ejercicio-actividad-a-completed-message"> ✓</span>}
                        </div>
                        
                        {/* Botón y panel de información */}
                        <div className="ejercicio-actividad-a-info-panel">
                        <button 
                            className="ejercicio-actividad-a-info-toggle-btn"
                            onClick={() => setShowInfoPanel(!showInfoPanel)}
                        >
                            {showInfoPanel ? 'Ocultar información' : 'Ver más información'}
                        </button>
                        
                        {showInfoPanel && (
                            <div className="ejercicio-actividad-a-info-content">
                            <div className="ejercicio-actividad-a-fen-display">
                                <h4>Posición FEN:</h4>
                                <p>{game.fen()}</p>
                                <button 
                                className="ejercicio-actividad-a-copy-btn"
                                onClick={() => {
                        ...
                        <button 
                            className={`ejercicio-actividad-a-submit-btn ${completed ? '' : 'disabled'}`}
                            disabled={!completed}
...
export default EjercicioActividadA;