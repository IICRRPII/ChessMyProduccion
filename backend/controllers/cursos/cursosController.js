...
const createCurso = async (req = request, res = response) => {
    ...
    try {
        ...
        try {
            ...
            const user = await Usuario.findOne({ where: { idUsuario: idUsuario, isActive: null, rolUsuario: "maestro" } });
            if (!user) {
...
const getCursoByToken = async (req = request, res = response) => {
    try{
        const cursoToken  = req.params.token;   
        if (!cursoToken) {
            return res.status(400).json({
                message: 'El token del curso es requerido'
            });
        }

        const curso = await Curso.findOne({where: {cursoToken: cursoToken}})
        if (!curso) {
            return res.status(404).json({
                message: 'No se encontró ningún curso con ese token'
            });
        }
        ...
        return res.status(500).json({ 
            message: 'Error al obtener el curso por token',
            error: error.message 
        });
    }
}
...
const showCurso = async(req = request,res = response) => {
    try{
        const curso = await Curso.findOne({ where: { idCurso: req.params.id } });
         
        if (!curso) {
            return res.status(404).json({ 
                success: false,
                message: 'Curso no encontrado con el ID proporcionado' 
            });
        }
...
const registerAlumnoToCurso = async(req = request, res = response) => {
    try {
        ...
        const user = await Usuario.findOne({ where: { idUsuario: idUsuario, isActive: null } });
        if (!user) {
            return res.status(404).json({ 
                message: 'Usuario no encontrado o no está activo' 
            });
...
const showComentarios = async (req = request, res = response) => {
    ...
    try {
        const publicacionExistente = await PublicacionTablon.findOne({
            where: {
                idPubTablon: idpubtablon,
                estatusPublicacion: 'activo' // También puedes validar que esté activa
            }
        });
        ...
        if (!publicacionExistente) {
            return res.status(404).json({ 
                success: false,
                message: 'La publicación no existe o no está activa' 
            });
        }
...
const agregarEjerciciosEnElCurso = async (req = request, res = response) => {
    ...
    try {
        ...
        console.log('ejercicioRelaciom', ejercicioRelacion)
...
const verEjerciciosAlumno = async (req = request, res = response) => {
    ...
    try {
        ...
        const ejerciciosAlumno = await EjerciciosAlumnos.findAll({
            where: { idUsuario },
            attributes: ['idEjercicioCurso'] // Solo necesitamos los IDs de las tareas entregadas
        });

        return res.status(200).json(ejerciciosAlumno.map(e => e.idEjercicioCurso));
    } catch(error) {
...
const verEjerciciosDelUsuarioEnElCurso = async (req = request, res = response) => {
    ...
    try {
        if (!idCurso || !idUsuario) {
            return res.status(400).json({ 
                success: false,
                message: 'Se requieren ambos parámetros: idCurso e idUsuario' 
            });
        }
        ...
        const ejerciciosCurso = await EjerciciosCurso.findAll({
            where: { idCurso },
            attributes: ['idEjercicioCurso', 'descripcion']
        });
        ...
        // Mapeo de ID a descripciones
        const mapaEjercicios = {};
        ejerciciosCurso.forEach(e => {
            mapaEjercicios[e.idEjercicioCurso] = e.descripcion;
        });
        ...
        // Formatear la respuesta manualmente con los datos del mapa
        const resultados = ejerciciosAlumno.map(ejercicio => ({
            ...
            descripcion: mapaEjercicios[ejercicio.idEjercicioCurso] || null
        }));
    ...
    try {
        ...
        idsEjercicios.forEach(id => {
            ...
            respuesta.ejercicios[id] = {
                ...
                idEjercicio: relacion.idEjercicio,
