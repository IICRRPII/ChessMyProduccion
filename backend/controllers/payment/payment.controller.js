...
const searchPago = async (req = request, res = response) => {
    try{
        const user = await Usuario.findOne({ where: { idUsuario: req.params.id } })
        if (!user) {
        ...
        // 3. Buscar los pagos del usuario
        const pagos = await Pago.findAll({ 
            where: { idUsuario: req.params.id }
        });

        // 4. Manejar el caso cuando no hay pagos (findAll devuelve array vacío, no null)
        if (pagos.length === 0) {
        ...
        // 5. Retornar los pagos encontrados
        return res.status(200).json({
            success: true,
            data: pagos
        });
...
module.exports = { createOrder, captureOrder, cancelPayment, getPlans, showPagos, searchPago };